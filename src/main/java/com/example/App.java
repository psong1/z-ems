...
public class App {
    ...
    private static void handleAuth(Context ctxt) {
        ...
        try {
            boolean valid = UserAuth.authenticate(email, pw);
            if (!valid) {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            switch (action) {
                case "addEmployee" -> {
                    ...
                    String role     = ctxt.queryParam("role");
                    adminAccess.submitNewEmployee(empid, fname, lname, hd, salary, ssn, usern, passw, role);
    ...
    private static void handleEmployee(Context ctxt) {
        String action = ctxt.queryParam("action");
        Integer empid = ctxt.queryParamAsClass("empid", Integer.class).getOrDefault(null);
        Integer currentEmpId = ctxt.sessionAttribute("empid");
